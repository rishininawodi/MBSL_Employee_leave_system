document.addEventListener('DOMContentLoaded', () => {
  const viewMode = document.body.dataset.view || 'pending';
  ...
  const filterParam = viewMode === 'history' ? 'history' : 'pending';
  ...
  async function loadRequests() {
    try {
      const res = await fetch(`/MBSL_Employee_leave_system/controllers/get_leave_requests.php?filter=${encodeURIComponent(filterParam)}`, {
  ...
  function getFilteredRequests() {
    ...
    if (viewMode === 'history') {
      ...
      if (!currentFilter) return historyRequests;
      return historyRequests.filter((req) => req.status.toLowerCase() === currentFilter);
    }
  ...
  function renderRequests() {
    ...
    if (!filtered.length) {
      const emptyLabel = viewMode === 'pending' ? 'pending employee requests' : 'history requests';
      requestsList.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><i class="bi bi-inbox"></i></div><p>No ' + emptyLabel + ' found</p></div>';
      return;
    }

    requestsList.innerHTML = filtered.map((req) => {
      const showActions = viewMode === 'pending';
      const statusLabel = req.status === 'Pending' ? 'Awaiting review' : req.status === 'Approved' ? 'Approved request' : 'Rejected request';
      return `
        <div class="request-item ${req.status.toLowerCase()}">
          <div class="request-info">
            ...
            <div style="font-size:0.82rem; color:#6a858d; margin-top:-2px; margin-bottom:8px;">${statusLabel}</div>
            ...
            <div class="request-footer">
              <div class="request-meta">
                ...
                <span><i class="bi bi-bar-chart me-1"></i>Remaining: ${req.remaining_days} days</span>
              </div>
  ...
  window.viewDetails = (requestId) => {
    const req = allRequests.find((item) => item.id === requestId);
    if (!req || !modalContent || !approvalModal) return;
      ...
      <div style="margin-bottom: 16px;">
        ...
        <div style="background: #f9fafb; padding: 12px; border-radius: 10px; margin-bottom: 12px;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 0.9rem;">
            ...
            <div><span style="color: #6a858d;">Remaining:</span> <strong>${req.remaining_days} days</strong></div>
          </div>
        </div>
    ...
    if (approveBtn) approveBtn.style.display = req.status === 'Pending' && viewMode === 'pending' ? 'block' : 'none';
    if (rejectBtn) rejectBtn.style.display = req.status === 'Pending' && viewMode === 'pending' ? 'block' : 'none';
  ...
  // Update avatar click to show profile
  if (rpAvatar) {
